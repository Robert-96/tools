...
from src.regexutils import _ranges_overlap
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (5, 6, 1, 5, True), # Touching at the end
        (5, 10, 10, 15, True), # Touching at the start
        (0, 10, 5, 15, True), # One range inside another
    ]
)
def test_ranges_overlap(start1, end1, start2, end2, expected):
    assert _ranges_overlap(start1, end1, start2, end2) == expected
